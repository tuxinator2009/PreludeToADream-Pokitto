...
Audio::RAWFileSource *PTAD::Music::backgroundMusic = nullptr;
Audio::RAWFileSource *PTAD::Music::sfxSource = nullptr;
Audio::Sink<2, PROJ_AUD_FREQ> PTAD::Music::audio;
...
void PTAD::Music::playMusic(uint8_t bgm, bool loop)
{
  ...
  if (bgm != currentBGM)
  {
    ...
    backgroundMusic = &Audio::play<0>(PTAD::dataFile->getFile(), file.size());
  }
...
void PTAD::Music::playSFX(uint8_t sfxID)
{
  DataPack::PackedFile file;
  Audio::stop<1>();
  PTAD::dataFile->getPackedFile(sfx[sfxID & 0x7F], &file);
  PTAD::dataFile->readyRead(file);
  sfxSource = &Audio::play<1>(PTAD::dataFile->getFile(), file.size());
